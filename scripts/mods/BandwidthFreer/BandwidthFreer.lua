local mod = get_mod("BandwidthFreer")
...
mod.on_all_mods_loaded = function()
	...
	local MorePlayers2 = get_mod("MorePlayers2")
	if MorePlayers2 then
		mod:echo("BandwidthFreer disabled itself as BTMP already have an integrated version of it")
		return
	end
	local Peregrinaje = get_mod("Peregrinaje")
	if Peregrinaje then
		mod:echo("BandwidthFreer disabled itself as Peregrinaje already have an integrated version of it")
		return
	end
	...
	mod:hook_origin(GameNetworkManager, "anim_event", function(self, unit, event)
		...
		if self.game_session then
			if self.is_server then
				...
				if not actual_anim[unit] or (time - actual_anim[unit].timer) > 0.5 or actual_anim[unit].event_id ~= event_id then
